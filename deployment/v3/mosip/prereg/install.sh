...
function installing_prereg() {
  ...
  helm -n $NS install prereg-captcha mosip/prereg-captcha --version $CHART_VERSION --set image.repository=dharanidharan0411/pre-registration-captcha-service --set image.tag=tf_myanmar

  echo Installing prereg-application
  helm -n $NS install prereg-application mosip/prereg-application --version $CHART_VERSION --set image.repository=dharanidharan0411/pre-registration-application-service --set image.tag=tf_myanmar
  ...
  helm -n $NS install prereg-datasync mosip/prereg-datasync --version $CHART_VERSION  --set image.repository=dharanidharan0411/pre-registration-datasync-service --set image.tag=tf_myanmar
  ...
  helm -n $NS install prereg-ui mosip/prereg-ui --set prereg.apiHost=$PREREG_HOST --version $CHART_VERSION --set image.repository=dharanidharan0411/pre-registration-ui --set image.tag=tf_myanmar
